...
def transfer_fund_code(spark, data_df, tx_dt):
    ...
    based_df = data_df.join(fund_portfolio, on=['robin_fund_code'], how="left") \
        .checkpoint()

    original_df = based_df.where(col("fund_code").isNotNull()) \
        .select(cols) \
        .checkpoint()

    new_df = based_df.where(col("fund_code").isNull()) \
        .drop("fund_code") \
